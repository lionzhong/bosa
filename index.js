...
const saveCsv = require('./modules/saveCsv');

const filePath = path.normalize(path.join(config.folder, `${config.fileName}.${config.fileType}`));

if (fs.existsSync(filePath)) {

    let originalData = fs.readFileSync(`./data/original.txt`, 'utf-8');
    ...
    if (originalData.length <= 0) {
        ...
        let result = [JSON.parse(JSON.stringify(Object.assign({}, meta, { "id": "证件号码" })))];
        ...
        util.output.json(`./data/original/test.json`, result);
